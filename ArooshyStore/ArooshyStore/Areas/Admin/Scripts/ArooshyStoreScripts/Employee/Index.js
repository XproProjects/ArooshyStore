$(function () {
    ...
    $('#myTable').dataTable({
        ...
        "columns": [
            ...
            {
                "data": "EmployeeId", "width": "130px", "class": "Acenter", "orderable": false, "render": function (data) {
                    var edit = '<a title="Edit Employee" data-toggle="modal" data-target="#MyModal" class="AddEditRecord btn btn-primary btnOpenModal" href="javascript:void(0)" data-value="' + data + '" style="border-radius:20px;padding:5px;color:#fff;padding-left:15px;padding-right:15px"><i class="fas fa-fw fa-edit"></i></a>&nbsp';
                    var del = '<a title="Delete Employee" class="DeleteRecord btn btn-danger" data-toggle="modal" data-target="#DeleteModal" href="javascript:void(0)" data-value="' + data + '" style="border-radius:20px;padding:5px;color:#fff;padding-left:15px;padding-right:15px"><i class="fas fa-fw fa-times-circle"></i></a>&nbsp';
                    var final = '';
...
$(document).on('click', '.AddEditRecord', function () {
    ...
    if (id > 0) {
        $('#ModelHeaderSpan').html('Edit Employee');
    }
    else {
        $('#ModelHeaderSpan').html('Add Employee');
    }
    $('#modalDiv').html('');
    // $('#modalDiv').load('@Url.Action("InsertUpdateCity", "Employee")?id=' + id + '');
    $.ajax({
        type: "GET",
...
function SearchItem() {
    ...
    if (BElement.html() == 'Employee Name') {
...
$(document).on('click', '.DeleteRecord', function () {
    var id = $(this).attr("data-value");
    $('#DeleteModalTitle').html("Delete Employee");
    $('#DeleteModalBody').html("Are you sure you want to delete this Employee?");
...
function DeleteRecord(id) {
    $.ajax({
        url: '/Admin/Employee/DeleteEmployee/' + id,
        type: "POST",
        dataType: 'json',
