$(function () {
    ...
    $('#myTable').dataTable({
        ...
        "ajax": {
            "url": "/Admin/ExpenseType/GetAllExpenseTypes",
            "type": "POST",
            "datatype": "json",
        ...
        "columns": [
            ...
            {
                "data": "ExpenseTypeId", "width": "130px", "class": "Acenter", "orderable": false, "render": function (data) {
                    var edit = '<a title="Edit Expense Type" data-toggle="modal" data-target="#MyModal" class="AddEditRecord btn btn-primary btnOpenModal" href="javascript:void(0)" data-value="' + data + '" style="border-radius:20px;padding:5px;color:#fff;padding-left:15px;padding-right:15px"><i class="fas fa-fw fa-edit"></i></a>&nbsp';
                    var del = '<a title="Delete Expense Type" class="DeleteRecord btn btn-danger" data-toggle="modal" data-target="#DeleteModal" href="javascript:void(0)" data-value="' + data + '" style="border-radius:20px;padding:5px;color:#fff;padding-left:15px;padding-right:15px"><i class="fas fa-fw fa-times-circle"></i></a>&nbsp';
                    var final = '';
...
$(document).on('click', '.AddEditRecord', function () {
    ...
    if (id > 0) {
        $('#ModelHeaderSpan').html('Edit Expense Type');
    }
    else {
        $('#ModelHeaderSpan').html('Add Expense Type');
    }
    $('#modalDiv').html('');
    // $('#modalDiv').load('@Url.Action("InsertUpdateExpenseType", "ExpenseType")?id=' + id + '');
    $.ajax({
        type: "GET",
...
function SearchItem() {
    ...
    if (BElement.html() == 'Expense Type Name') {
...
$(document).on('click', '.DeleteRecord', function () {
    var id = $(this).attr("data-value");
    $('#DeleteModalTitle').html("Delete Expense Type");
    $('#DeleteModalBody').html("Are you sure you want to delete this Expense Type?");
...
function DeleteRecord(id) {
    $.ajax({
        url: '/Admin/ExpenseType/DeleteExpenseType/' + id,
        type: "POST",
        dataType: 'json',
