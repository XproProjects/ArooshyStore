            ...
            { "data": "UpdatedByString", "name": "UpdatedBy", "autoWidth": true },
            {
                "data": "InvoiceNumber", "width": "180px", "class": "Acenter", "orderable": false, "autoWidth": false, "render": function (data) {
                    ...
                    if ($('#EditActionRole').val() > 0) {
                        div += '<a class="dropdown-item AddEditRecord btnOpenModal btnAddEdit" href="javascript:void(0)" data-value="' + data + '" title="Edit Sale Return"  style="text-decoration:none !important;font-weight:normal !important">Edit</a>';
                    }
                    if ($('#DeleteActionRole').val() > 0) {
                        div += '<a class="dropdown-item DeleteRecord" href="javascript:void(0)" title="Delete Sale Return" data-toggle="modal" data-target="#DeleteModal"  data-value="' + data + '" style="text-decoration:none !important;font-weight:normal !important">Delete</a>';
                    }
                    div += '</div>' +
                        '</div>';
...
$(document).on('click', '.btnAddEdit', function () {
    ...
    window.location.href = "/Admin/invoice/insertupdatesalereturn/?id=" + id + "";
...
$(document).on('click', '.DeleteRecord', function () {
    var id = $(this).attr("data-value");
    $('#DeleteModalTitle').html("Delete Sale Return");
    $('#DeleteModalBody').html("Are you sure you want to delete this Sale Return?");
