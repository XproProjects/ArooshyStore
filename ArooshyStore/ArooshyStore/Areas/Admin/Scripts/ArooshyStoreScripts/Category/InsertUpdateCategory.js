...
$(function () {
    if ($("#From").val().toLowerCase() == "child") {
        ...
        if ($('#CategoryId').val() > 0) {
            if ($('#ParentCategoryId').find("option[value='" + $('#HiddenParentCategoryId').val() + "']").length) {
                $('#ParentCategoryId').val($('#HiddenParentCategoryId').val()).trigger('change');
            } else {
                // Create a DOM Option and pre-select by default
                var newOption = new Option($('#HiddenParentCategoryName').val(), $('#HiddenParentCategoryId').val(), true, true);
                // Append it to the select
                $('#ParentCategoryId').append(newOption).trigger('change');
            }
        }
        else {
            $('#ParentCategoryId').val(null).trigger('change');
        }
    }
                        ...
                        success: function (data) {
                            if (data.status == true) {
                                ...
                                xhr.onload = function (e) {
                                    ...
                                    toastr.success("Category Saved Successfully", "Success", { timeOut: 3000, "closeButton": true });
                                    $('.close').click();
