$(function () {
    ...
    $('#myTable').dataTable({
        ...
        "columns": [
            ...
            {
                "data": "UnitId", "width": "130px", "class": "Acenter", "orderable": false, "render": function (data) {
                    var edit = '<a title="Edit Unit" data-toggle="modal" data-target="#MyModal" class="AddEditRecord btn btn-primary btnOpenModal" href="javascript:void(0)" data-value="' + data + '" style="border-radius:20px;padding:5px;color:#fff;padding-left:15px;padding-right:15px"><i class="fas fa-fw fa-edit"></i></a>&nbsp';
                    var del = '<a title="Delete Unit" class="DeleteRecord btn btn-danger" data-toggle="modal" data-target="#DeleteModal" href="javascript:void(0)" data-value="' + data + '" style="border-radius:20px;padding:5px;color:#fff;padding-left:15px;padding-right:15px"><i class="fas fa-fw fa-times-circle"></i></a>&nbsp';
                    var final = '';
...
$(document).on('click', '.AddEditRecord', function () {
    ...
    if (id > 0) {
        $('#ModelHeaderSpan').html('Edit Unit');
    }
    else {
        $('#ModelHeaderSpan').html('Add Unit');
    }
    $('#modalDiv').html('');
    // $('#modalDiv').load('@Url.Action("InsertUpdateUnit", "Unit")?id=' + id + '');
    $.ajax({
        type: "GET",
...
$(document).on('click', '.DeleteRecord', function () {
    var id = $(this).attr("data-value");
    $('#DeleteModalTitle').html("Delete Unit");
    $('#DeleteModalBody').html("Are you sure you want to delete this Unit?");
