$(function () {
    ...
    $('#myTable').dataTable({
        ...
        "columns": [
            ...
            {
                "data": "CarouselId", "width": "130px", "class": "Acenter", "orderable": false, "render": function (data) {
                    var edit = '<a title="Edit Carousel" data-toggle="modal" data-target="#MyModal" class="AddEditRecord btn btn-primary btnOpenModal" href="javascript:void(0)" data-value="' + data + '" style="border-radius:20px;padding:5px;color:#fff;padding-left:15px;padding-right:15px"><i class="fas fa-fw fa-edit"></i></a>&nbsp';
                    var del = '<a title="Delete Carousel" class="DeleteRecord btn btn-danger" data-toggle="modal" data-target="#DeleteModal" href="javascript:void(0)" data-value="' + data + '" style="border-radius:20px;padding:5px;color:#fff;padding-left:15px;padding-right:15px"><i class="fas fa-fw fa-times-circle"></i></a>&nbsp';
                    var final = '';
...
$(document).on('click', '.AddEditRecord', function () {
    ...
    if (id > 0) {
        $('#ModelHeaderSpan').html('Edit Carousel');
    }
    else {
        $('#ModelHeaderSpan').html('Add Carousel');
    }
    $('#modalDiv').html('');
    // $('#modalDiv').load('@Url.Action("InsertUpdateCarousel", "Carousel")?id=' + id + '');
    $.ajax({
        type: "GET",
...
$(document).on('click', '.DeleteRecord', function () {
    var id = $(this).attr("data-value");
    $('#DeleteModalTitle').html("Delete Carousel");
    $('#DeleteModalBody').html("Are you sure you want to delete this Carousel?");
...
function DeleteRecord(id) {
    $.ajax({
        url: '/Admin/Carousel/DeleteCarousel/' + id,
        type: "POST",
        dataType: 'json',
