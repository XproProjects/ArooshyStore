$(function () {
    $('#myTable').dataTable({
        ...
        "columns": [
            ...
            {
                "data": "ModuleId", "width": "130px", "class": "Acenter", "orderable": false, "render": function (data) {
                    var edit = '<a title="Edit Module" data-toggle="modal" data-target="#MyModal" class="AddEditRecord btn btn-primary btnOpenModal" href="javascript:void(0)" data-value="' + data + '" style="border-radius:20px;padding:5px;color:#fff;padding-left:15px;padding-right:15px"><i class="fas fa-fw fa-edit"></i></a>&nbsp';
                    var del = '<a title="Delete Module" class="DeleteRecord btn btn-danger" data-toggle="modal" data-target="#DeleteModal" href="javascript:void(0)" data-value="' + data + '" style="border-radius:20px;padding:5px;color:#fff;padding-left:15px;padding-right:15px"><i class="fas fa-fw fa-times-circle"></i></a>&nbsp';
                    var final = '';
                    final = final + edit;
                    final = final + del;
                    return final;
...
$(document).on('click', '.AddEditRecord', function () {
    ...
    if (id > 0) {
        $('#ModelHeaderSpan').html('Edit Module');
    }
    else {
        $('#ModelHeaderSpan').html('Add Module');
    }
    $('#modalDiv').html('');
    // $('#modalDiv').load('@Url.Action("InsertUpdateModule", "Module")?id=' + id + '');
    $.ajax({
        type: "GET",
...
$(document).on('click', '.DeleteRecord', function () {
    var id = $(this).attr("data-value");
    $('#DeleteModalTitle').html("Delete Module");
    $('#DeleteModalBody').html("Are you sure you want to delete this Module?");
...
function DeleteRecord(id) {
    $.ajax({
        url: '/Admin/Module/DeleteModule/' + id,
        type: "POST",
        dataType: 'json',
