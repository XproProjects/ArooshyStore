...
namespace ArooshyStore.Areas.Admin.Controllers
{
    public class WarehouseController : BaseController
    {
        ...
        public WarehouseController(IWarehouseRepository repository, ICheckUserRoleRepository roles)
        {
        ...
        public ActionResult Index()
        {
            if (User != null)
            {
                ...
                if (_roles.CheckModuleRoleId(User.UserId, "warehouse") > 0)
                {
                    //Get list of all actions of this module
                    List<ModuleViewModel> actionList = new List<ModuleViewModel>();
                    actionList = _roles.ActionsList(User.UserId, "warehouse");
        ...
        public ActionResult GetAllWarehouses()
        {
            if (User != null)
            {
                ...
                {
                    sorting = " Order by s.WarehouseId asc";
                }
                if (!(string.IsNullOrEmpty(userName)))
                {
                    whereCondition += " LOWER(s.WarehouseName) like ('%" + userName.ToLower() + "%')";
                }
                else if (!(string.IsNullOrEmpty(Email)))
                {
                    whereCondition += " LOWER(s.Email) like ('%" + Email.ToLower() + "%')";
                }
                else
                {
                    whereCondition += " LOWER(s.WarehouseName) like ('%%')";
                }
                List<WarehouseViewModel> listsub = new List<WarehouseViewModel>();
                if (_roles.CheckActionRoleId(User.UserId, "warehouse", "view") > 0)
                {
                    listsub = _repository.GetWarehousesListAndCount(whereCondition, start, length, sorting);
                    if (listsub.Count > 0)
                    {
        ...
        [HttpGet]
        public ActionResult InsertUpdateWarehouse(int id = 0)
        {
            if (User != null)
            {
                WarehouseViewModel user = _repository.GetWarehouseById(id);
        ...
        [HttpPost]
        public ActionResult InsertUpdateWarehouse(WarehouseViewModel user)
        {
            StatusMessageViewModel response = new StatusMessageViewModel();
            if (User != null)
            {
                response = _repository.InsertUpdateWarehouse(user, User.UserId);
            }
            else
            {
        ...
        public ActionResult DeleteWarehouse(int id)
        {
            ...
            if (User != null)
            {
                WarehouseViewModel user = new WarehouseViewModel();
                user.WarehouseId = id;
                response = _repository.DeleteWarehouse(id, User.UserId);
            }
            else
            {
                BusinessInfo business = BusinessInfo.GetInstance;
