...
namespace ArooshyStore.Areas.Admin.Controllers
{
    public class ExpenseTypeController : BaseController
    {
        ...
        public ExpenseTypeController(IExpenseTypeRepository repository, ICheckUserRoleRepository roles)
        {
        ...
        public ActionResult Index()
        {
            if (User != null)
            {
                ...
                if (_roles.CheckModuleRoleId(User.UserId, "expense type") > 0)
                {
                    //Get list of all actions of this module
                    List<ModuleViewModel> actionList = new List<ModuleViewModel>();
                    actionList = _roles.ActionsList(User.UserId, "expense type");
        ...
        // GET: /Home/
        public ActionResult GetAllExpenseTypes()
        {
            if (User != null)
            {
                ...
                {
                    sorting = " Order by s.ExpenseTypeId asc";
                }
                if (!(string.IsNullOrEmpty(userName)))
                {
                    whereCondition += " LOWER(s.TypeName) like ('%" + userName.ToLower() + "%')";
                }
               
                else
                {
                    whereCondition += " LOWER(s.TypeName) like ('%%')";
                }
                List<ExpenseTypeViewModel> listsub = new List<ExpenseTypeViewModel>();
                if (_roles.CheckActionRoleId(User.UserId, "expense type", "view") > 0)
                {
                    listsub = _repository.GetExpenseTypesListAndCount(whereCondition, start, length, sorting);
                    if (listsub.Count > 0)
                    {
        ...
        [HttpGet]
        public ActionResult InsertUpdateExpenseType(int id = 0)
        {
            if (User != null)
            {
                ExpenseTypeViewModel user = _repository.GetExpenseTypeById(id);
        ...
        [HttpPost]
        public ActionResult InsertUpdateExpenseType(ExpenseTypeViewModel user)
        {
            StatusMessageViewModel response = new StatusMessageViewModel();
            if (User != null)
            {
                response = _repository.InsertUpdateExpenseType(user, User.UserId);
            }
            else
            {
        ...
        public ActionResult DeleteExpenseType(int id)
        {
            ...
            if (User != null)
            {
                ExpenseTypeViewModel user = new ExpenseTypeViewModel();
                user.ExpenseTypeId = id;
                response = _repository.DeleteExpenseType(id, User.UserId);
            }
            else
            {
                BusinessInfo business = BusinessInfo.GetInstance;
