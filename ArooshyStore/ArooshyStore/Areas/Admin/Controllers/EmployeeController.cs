...
namespace ArooshyStore.Areas.Admin.Controllers
{
    public class EmployeeController : BaseController
    {
        ...
        public EmployeeController(IEmployeeRepository repository, ICheckUserRoleRepository roles)
        {
        ...
        public ActionResult Index()
        {
            if (User != null)
            {
                ...
                if (_roles.CheckModuleRoleId(User.UserId, "employee") > 0)
                {
                    //Get list of all actions of this module
                    List<ModuleViewModel> actionList = new List<ModuleViewModel>();
                    actionList = _roles.ActionsList(User.UserId, "employee");
        ...
        public ActionResult GetAllEmployees()
        {
            if (User != null)
            {
                ...
                {
                    sorting = " Order by s.EmployeeId asc";
                }
                if (!(string.IsNullOrEmpty(userName)))
                {
                    whereCondition += " LOWER(s.EmployeeName) like ('%" + userName.ToLower() + "%')";
                }
               
                else
                {
                    whereCondition += " LOWER(s.EmployeeName) like ('%%')";
                }
                List<EmployeeViewModel> listsub = new List<EmployeeViewModel>();
                if (_roles.CheckActionRoleId(User.UserId, "employee", "view") > 0)
                {
                    listsub = _repository.GetEmployeesListAndCount(whereCondition, start, length, sorting);
                    if (listsub.Count > 0)
                    {
        ...
        [HttpGet]
        public ActionResult InsertUpdateEmployee(int id = 0)
        {
            if (User != null)
            {
                EmployeeViewModel user = _repository.GetEmployeeById(id);
        ...
        [HttpPost]
        public ActionResult InsertUpdateEmployee(EmployeeViewModel user)
        {
            StatusMessageViewModel response = new StatusMessageViewModel();
            if (User != null)
            {
                response = _repository.InsertUpdateEmployee(user, User.UserId);
            }
            else
            {
        ...
        public ActionResult DeleteEmployee(int id)
        {
            ...
            if (User != null)
            {
                ...
                user.EmployeeId = id;
                response = _repository.DeleteEmployee(id, User.UserId);
            }
            else
            {
                BusinessInfo business = BusinessInfo.GetInstance;
