...
namespace ArooshyStore.Areas.Admin.Controllers
{
    public class UserTypeController : BaseController
    {
        ...
        public UserTypeController(IUserTypeRepository repository, ICheckUserRoleRepository roles)
        {
        ...
        public ActionResult Index()
        {
            if (User != null)
            {
                ...
                if (_roles.CheckModuleRoleId(User.UserId, "user type") > 0)
                {
                    //Get list of all actions of this module
                    List<ModuleViewModel> actionList = new List<ModuleViewModel>();
                    actionList = _roles.ActionsList(User.UserId, "user type");
        ...
        public ActionResult GetAllUserTypes()
        {
            if (User != null)
            {
                ...
                int recordsTotal = 0;
                string whereCondition = " isnull(s.TypeName,'') != 'Super Admin' ";
                string sorting = "";
                ...
                {
                    sorting = " Order by s.UserTypeId asc";
                }
                if (!(string.IsNullOrEmpty(userName)))
                {
                    whereCondition += " and LOWER(s.TypeName) like ('%" + userName.ToLower() + "%')";
                }
                else
                {
                    whereCondition += " and LOWER(s.TypeName) like ('%%')";
                }
                List<UserTypeViewModel> listsub = new List<UserTypeViewModel>();
                if (_roles.CheckActionRoleId(User.UserId, "user type", "view") > 0)
                {
                    listsub = _repository.GetUserTypesListAndCount(whereCondition, start, length, sorting);
                    if (listsub.Count > 0)
                    {
        ...
        [HttpGet]
        public ActionResult InsertUpdateUserType(int id = 0)
        {
            if (User != null)
            {
                UserTypeViewModel user = _repository.GetUserTypeById(id);
        ...
        [HttpPost]
        public ActionResult InsertUpdateUserType(UserTypeViewModel user)
        {
            StatusMessageViewModel response = new StatusMessageViewModel();
            if (User != null)
            {
                response = _repository.InsertUpdateUserType(user, User.UserId);
            }
            else
            {
        ...
        public ActionResult DeleteUserType(int id)
        {
            ...
            if (User != null)
            {
                UserTypeViewModel user = new UserTypeViewModel();
                user.UserTypeId = id;
                response = _repository.DeleteUserType(id, User.UserId);
            }
            else
            {
                BusinessInfo business = BusinessInfo.GetInstance;
