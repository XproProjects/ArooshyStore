...
namespace ArooshyStore.Areas.Admin.Controllers
{
    public class ExpenseController : BaseController
    {
        ...
        public ExpenseController(IExpenseRepository repository, ICheckUserRoleRepository roles)
        {
        ...
        public ActionResult Index()
        {
            if (User != null)
            {
                ...
                if (_roles.CheckModuleRoleId(User.UserId, "expense") > 0)
                {
                    //Get list of all actions of this module
                    List<ModuleViewModel> actionList = new List<ModuleViewModel>();
                    actionList = _roles.ActionsList(User.UserId, "expense");
        ...
        public ActionResult GetAllExpenses()
        {
            if (User != null)
            {
                ...
                {
                    sorting = " Order by s.ExpenseId asc";
                }
                if (!(string.IsNullOrEmpty(userName)))
                {
                    whereCondition += " LOWER(s.ExpenseName) like ('%" + userName.ToLower() + "%')";
                }
                else
                {
                    whereCondition += " LOWER(s.ExpenseName) like ('%%')";
                }
                List<ExpenseViewModel> listsub = new List<ExpenseViewModel>();
                if (_roles.CheckActionRoleId(User.UserId, "expense", "view") > 0)
                {
                    listsub = _repository.GetExpensesListAndCount(whereCondition, start, length, sorting);
                    if (listsub.Count > 0)
                    {
        ...
        [HttpGet]
        public ActionResult InsertUpdateExpense(int id = 0)
        {
            if (User != null)
            {
                ExpenseViewModel user = _repository.GetExpenseById(id);
        ...
        [HttpPost]
        public ActionResult InsertUpdateExpense(ExpenseViewModel user)
        {
            StatusMessageViewModel response = new StatusMessageViewModel();
            if (User != null)
            {
                response = _repository.InsertUpdateExpense(user, User.UserId);
            }
            else
            {
        ...
        public ActionResult DeleteExpense(int id)
        {
            ...
            if (User != null)
            {
                ExpenseViewModel user = new ExpenseViewModel();
                user.ExpenseId = id;
                response = _repository.DeleteExpense(id, User.UserId);
            }
            else
            {
                BusinessInfo business = BusinessInfo.GetInstance;
