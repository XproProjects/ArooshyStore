...
namespace ArooshyStore.Areas.Admin.Controllers
{
    public class DesignationController : BaseController
    {
        ...
        public DesignationController(IDesignationRepository repository, ICheckUserRoleRepository roles)
        {
        ...
        public ActionResult Index()
        {
            if (User != null)
            {
                ...
                if (_roles.CheckModuleRoleId(User.UserId, "designation") > 0)
                {
                    //Get list of all actions of this module
                    List<ModuleViewModel> actionList = new List<ModuleViewModel>();
                    actionList = _roles.ActionsList(User.UserId, "designation");
        ...
        public ActionResult GetAllDesignations()
        {
            if (User != null)
            {
                ...
                {
                    sorting = " Order by s.DesignationId asc";
                }
                if (!(string.IsNullOrEmpty(userName)))
                {
                    whereCondition += " LOWER(s.DesignationName) like ('%" + userName.ToLower() + "%')";
                }
                else
                {
                    whereCondition += " LOWER(s.DesignationName) like ('%%')";
                }
                List<DesignationViewModel> listsub = new List<DesignationViewModel>();
                if (_roles.CheckActionRoleId(User.UserId, "designation", "view") > 0)
                {
                    listsub = _repository.GetDesignationsListAndCount(whereCondition, start, length, sorting);
                    if (listsub.Count > 0)
                    {
        ...
        [HttpGet]
        public ActionResult InsertUpdateDesignation(int id = 0)
        {
            if (User != null)
            {
                DesignationViewModel user = _repository.GetDesignationById(id);
        ...
        [HttpPost]
        public ActionResult InsertUpdateDesignation(DesignationViewModel user)
        {
            StatusMessageViewModel response = new StatusMessageViewModel();
            if (User != null)
            {
                response = _repository.InsertUpdateDesignation(user, User.UserId);
            }
            else
            {
        ...
        public ActionResult DeleteDesignation(int id)
        {
            ...
            if (User != null)
            {
                DesignationViewModel user = new DesignationViewModel();
                user.DesignationId = id;
                response = _repository.DeleteDesignation(id, User.UserId);
            }
            else
            {
                BusinessInfo business = BusinessInfo.GetInstance;
